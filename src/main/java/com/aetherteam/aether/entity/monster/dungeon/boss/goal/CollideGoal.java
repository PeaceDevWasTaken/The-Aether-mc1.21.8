    ...
    public void tick() {
        ...
        if (this.slider.level() instanceof ServerLevel serverLevel) {
            for (Entity entity : this.slider.level().getEntities(this.slider, collisionBounds)) {
                if (entity instanceof LivingEntity livingEntity && entity.hurtServer(serverLevel, AetherDamageTypes.entityDamageSource(this.slider.level(), AetherDamageTypes.CRUSH, this.slider), 6)) {
