...
public class AetherJadePlugin implements IWailaPlugin {
    ...
    private Block getLockedDungeonBlock(String name) {
        if (name.startsWith("boss_doorway_")) {
            return BuiltInRegistries.BLOCK.getValue(ResourceLocation.fromNamespaceAndPath(Aether.MODID, "locked_" + name.substring(13)));
        } else if (name.startsWith("treasure_doorway_")) {
            return BuiltInRegistries.BLOCK.getValue(ResourceLocation.fromNamespaceAndPath(Aether.MODID, "locked_" + name.substring(17)));
        }
        return null;
    }
}
