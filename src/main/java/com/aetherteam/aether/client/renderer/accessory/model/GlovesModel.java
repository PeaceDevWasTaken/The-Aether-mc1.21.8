...
public class GlovesModel extends HumanoidModel<HumanoidRenderState> {
    ...
    public static LayerDefinition createLayer(CubeDeformation cube, boolean isSlim, boolean isTrim) {
        ...
        if (!isSlim) {
            partDefinition.addOrReplaceChild("right_arm", CubeListBuilder.create().texOffs(0, y).addBox(-3.0F, -2.0F, -2.0F, 4.0F, 12.0F, 4.0F, cube), PartPose.offset(-5.0F, 2.0F, 0.0F));
            partDefinition.addOrReplaceChild("left_arm", CubeListBuilder.create().texOffs(0, y).mirror().addBox(-1.0F, -2.0F, -2.0F, 4.0F, 12.0F, 4.0F, cube), PartPose.offset(5.0F, 2.0F, 0.0F));
        } else {
